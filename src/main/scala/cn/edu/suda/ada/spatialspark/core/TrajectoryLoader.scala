...
class HDFSTrajectoryLoader(val sc: SparkContext) extends TrajectoryLoader with Logging{
  ...
  override def loadTrajectoryFromDataSource(path: String): RDD[Trajectory] = {
    ...
    val rdd = lines.map(line => HDFSFileMapper.mapLine2Trajectory(line)).persist()
    rdd
  }

}
private[core] object HDFSFileMapper extends Serializable{
